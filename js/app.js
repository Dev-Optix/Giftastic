...
function renderButtons() {
 //renderButton will display buttons for intial array set
...
function getAnimalGifs() {
 ...
 var queryURL =
  ...
  addAttribute +
  "&api_key=d5589f089013444f8a5f800650fbf5a9&limit=10";

 $.ajax({
  //~~~~~~~~~~~~~~~~~~~~~~~~~AJAX calling to Giphy API
  method: "GET",
  url: queryURL,
...
function animateGif() {
 ...
 console.log(state);
 if (state === "still") {
  //~~~~~~~~~~~~if gif is still image switch to animated attr
  console.log(state);
 ...
 if (state !== "still") {
  //~~~~~~~~~~~~if gif is animated image switch to still attr
